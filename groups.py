...
This file is based on the uploaded script. Logging is configured at the top to write
to both stdout and a timestamped file in the `logs/` directory.
"""
...
# Place logging configuration here (immediately after imports) so all modules use it.
# This produces identical lines on console and in logs/<script>_<ts>.log.
# =========================
SCRIPT_NAME = "groups_batched"
LOG_DIR = "logs"
os.makedirs(LOG_DIR, exist_ok=True)
_ts = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
LOG_FILE = os.path.join(LOG_DIR, f"{SCRIPT_NAME}_{_ts}.log")

# Remove existing root handlers (if any) to avoid duplicate handlers.
root_logger = logging.getLogger()
root_logger.setLevel(logging.INFO)

# Clear handlers to avoid duplicates when script is reloaded.
...
# -----------------------------
# Configuration defaults & imports
# -----------------------------
...
# -----------------------------
# Utility helpers (commented)
# -----------------------------
def normalize_title(s: str) -> str:
    """Normalize a topic title for case- and whitespace-insensitive matching."""
    return " ".join(s.strip().lower().split())

async def handle_floodwait(e: errors.FloodWaitError):
    """Sleep for FloodWaitError.seconds + configured buffer."""
...
async def safe_invoke(*args, **kwargs):
    """
    Backward-compatible safe_invoke wrapper for Telethon calls.
    Accepts:
      safe_invoke(func, *fn_args)
      safe_invoke(client, func, *fn_args)
    Retries on transient errors and respects FloodWaitError handling.
    """
    from telethon import TelegramClient as _TC
    if not args:
        raise ValueError("safe_invoke requires at least one argument")
    # Support both calling conventions used in the script (client, method, ...) or (callable, ...)
...
async def list_forum_topics(client: TelegramClient, channel_entity) -> List[Dict[str, Any]]:
    """
    List forum topics for a channel using GetForumTopicsRequest.
    Returns a list of dicts: {'id': topic_id, 'title': title, 'top_message': top_message_id}
    """
    try:
...
async def ensure_target_topics(client: TelegramClient, target_entity, source_topics: List[Dict[str, Any]]) -> Dict[int, int]:
    """
    Ensure each source topic exists in target. If missing, create the topic.
    Return a mapping: source_topic_id -> target_topic_id
    """
...
async def build_top_message_map(client: TelegramClient, source_entity) -> Dict[int, int]:
    ...
    This helps resolving replies to topics when messages refer to top-message IDs.
    """
    mapping: Dict[int, int] = {}
    try:
...
async def resolve_topic_from_reply(client: TelegramClient, reply_msg_id: int, top_map: Dict[int,int], max_depth: int = 6) -> Optional[int]:
    ...
    Returns topic_id if found or None.
    """
    depth = 0
    cur_id = reply_msg_id
    try:
...
async def collect_topic_messages_single_pass(client: TelegramClient, source_entity, source_topic_id: int, top_map: Dict[int, int]):
    ...
    Note: forwarded messages are NOT skipped here (we intentionally migrate forwarded items).
    """
    seen_grouped = set()
    yield_count = 0
    checked = 0

    # visible start message in logs
    try:
        src_name = getattr(source_entity, "title", str(source_entity))
    except Exception:
        src_name = str(source_entity)
    log.info("Collector start for source='%s' topic_id=%s — beginning iter_messages scan", src_name, source_topic_id)
    ...
    async for msg in client.iter_messages(source_entity, reverse=True):
        ...
        if topic_for_msg is None:
            ...
            if rt_top is not None:
                try:
                    ...
                    pass
        # 2b) DIRECT TOPIC-ID REPLY (important for this dataset)
        # Some messages reply directly to the topic_id (not to top_message)
        if topic_for_msg is None:
            rt_msg_id = getattr(msg, "reply_to_msg_id", None)
            if rt_msg_id is not None:
                try:
                    rt_msg_int = int(rt_msg_id)
                   # If reply_to_msg_id equals the current topic_id, this message belongs to the topic
                    if rt_msg_int == source_topic_id:
                        topic_for_msg = source_topic_id
                        log.debug(
                        "Direct topic-id reply detected: msg=%s -> topic=%s",
                            getattr(msg, "id", None),
                            source_topic_id
                        )
                except Exception:
        ...
        if topic_for_msg != source_topic_id:
            # more frequent progress messages so operator sees activity
            if checked % 100 == 0:
                log.info("collect: checked %s msgs, found %s yields so far for topic %s (source=%s)", checked, yield_count, source_topic_id, src_name)
            continue

        # DO NOT skip forwarded messages — we want to migrate forwarded media too.
        ...
        if gid:
            ...
            try:
                ...
                log.info("Album window fetch failed for msg id=%s (topic=%s) — yielding single id", getattr(msg, "id", None), source_topic_id)
            ...
            log.info("Collector: found album for topic=%s at msg=%s (len=%d)", source_topic_id, getattr(msg, "id", None), len(ids))
        ...
        # single message
        if yield_count % 25 == 0:
            log.debug("Collector: emitting single msg id=%s topic=%s (checked=%s)", getattr(msg, "id", None), source_topic_id, checked)
        yield False, msg.id, getattr(msg, "message", "") or ""
        yield_count += 1

    log.info("Collector finished for topic %s: checked=%s yields=%s (source=%s)", source_topic_id, checked, yield_count, src_name)

# ---------------- helpers: file uid ----------------
def get_file_unique_id_from_msg(msg: Message) -> Optional[str]:
    """Return a stable file-unique id for media within a Message, used for dedupe."""
...
async def adaptive_forward(client: TelegramClient, target_entity, from_entity, msg_ids: List[int], target_topic_id: Optional[int] = None):
    """
    Forward a list of message ids to target. Attempt server-side forwarding first,
    then copy_messages, then fallback to reuploading via send_file/send_message.
    """
    ...
    # try copy_messages (server-side copy)
    try:
...
async def adaptive_forward_batch(client, target_entity, from_entity, ids_list: List[int], target_topic_id: Optional[int] = None):
    """Send ids_list in chunks respecting FORWARD_BATCH_SIZE and a safety cap of 200."""
    if not ids_list:
        return True
...
async def process_topic(client: TelegramClient, source_entity, target_entity, source_topic_id: int, target_topic_id: Optional[int]):
    """
    Process a single source topic:
      - build top_message map
      - iterate messages belonging to topic
      - batch forward / handle albums
      - persist forwarded file uids
    """
...
async def main():
    """
    Main entry: start Telethon client, resolve source/target entities,
    ensure topics exist in target, then process topics sequentially.
    """

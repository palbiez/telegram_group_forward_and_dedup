...
# Telethon 1.41-compatible adjustments:
# - use GetForumTopicsRequest from telethon.tl.functions.channels
# - create placeholder by client.send_message(..., reply_to=topic_id) (1.41 behavior)
# - resolve_topic_from_reply now requires source_entity to call get_messages correctly
# - fixed safe_invoke usage where it previously received client/req incorrectly
...
# Telethon 1.41: forum/topic functions live in channels
from telethon.tl.functions.channels import (
    CreateForumTopicRequest,
    GetForumTopicsRequest,  # <- moved here
)
# ForwardMessagesRequest is still a messages-level request
from telethon.tl.functions.messages import ForwardMessagesRequest
from telethon.tl.types import Message, InputReplyToMessage
import random

# user config import (adjusted to your local config module)
...
async def list_forum_topics(client: TelegramClient, channel_entity) -> List[Dict[str, Any]]:
    try:
        # telethon.channels.GetForumTopicsRequest expects parameter 'channel', not 'peer'
    ...
    for s in source_topics:
        ...
        if placeholder_id is None and tgt_id is not None:
            try:
                ...
                # Telethon 1.41 behaviour: sending a message that replies to the topic id associates it with that topic
                sent = await safe_invoke(client.send_message, target_entity, text, reply_to=tgt_id)
...
async def resolve_topic_from_reply(client: TelegramClient, source_entity, reply_msg_id: int, top_map: Dict[int,int], max_depth: int = 6) -> Optional[int]:
    depth = 0
    cur_id = reply_msg_id
    try:
        while cur_id is not None and depth < max_depth:
            try:
                m = await safe_invoke(client.get_messages, source_entity, ids=cur_id)
            except Exception:
                m = None
            if not m:
                return None
            # telethon message may have nested reply_to object; check multiple places
            # 1) direct forum_topic attribute (some versions)
            ...
            if reply_to is not None:
                ...
                if rt_top is not None:
                    try:
                        ...
                        pass
            # 3) reply_to_top_id on message object (older fields)
            rt_top2 = getattr(m, "reply_to_top_id", None)
            if rt_top2 is not None:
                try:
                    rt_top_int = int(rt_top2)
                    if rt_top_int in top_map:
                        return top_map[rt_top_int]
                except Exception:
                    pass
            # 4) msg.id itself is a top message
    ...
    async for msg in client.iter_messages(source_entity, reverse=True):
        ...
        if forum_topic:
            try:
                ...
                topic_for_msg = None
        # 2) reply_to.reply_to_top_id (via nested reply_to object)
            ...
            if reply_to is not None:
                ...
                if rt_top is not None:
                    try:
                        ...
                        pass
        # 3) msg.reply_to_top_id (alternate field)
        if topic_for_msg is None:
            rt_top2 = getattr(msg, "reply_to_top_id", None)
            if rt_top2 is not None:
                try:
                    rt_top_int = int(rt_top2)
                    if rt_top_int in top_map:
                    ...
                    pass
        # 4) reply_to_msg_id chain resolution
        if topic_for_msg is None:
            ...
            if rt_msg_id is not None:
                try:
                    ...
                    if rt_msg_int in top_map:
                        ...
                        resolved = await resolve_topic_from_reply(client, source_entity, rt_msg_int, top_map, max_depth=6)
                        if resolved is not None:
                            topic_for_msg = int(resolved)
                except Exception:
                    pass
        # 5) msg.id itself is top_message
        try:
...
async def forward_batch_as_reply(client: TelegramClient, from_entity, to_entity, msg_ids: List[int], placeholder_msg_id: Optional[int]):
    ...
    try:
        ...
        res = await safe_invoke(lambda *a, **k: client(req))  # wrap low-level request
